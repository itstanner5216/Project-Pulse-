#!/bin/bash
# Copilot CLI Wrapper with Project-Specific Hook Activation
...
main() {
    ...
    # Look for tool-related arguments
    for arg in "$@"; do
        case "$arg" in
    ...
    # Check GitHub Copilot specific patterns
    if [[ "$*" == *"github copilot"* ]] || [[ -n "${COPILOT_MODE:-}" ]]; then
        is_tool_call=1
    fi
    ...
    # Pass through to actual Copilot CLI
    if command -v github-copilot-cli >/dev/null 2>&1; then
        exec github-copilot-cli "$@"
    elif command -v copilot >/dev/null 2>&1; then
        exec copilot "$@"
    else
        echo "[Copilot Wrapper] Error: 'github-copilot-cli' or 'copilot' command not found" >&2
        echo "[Copilot Wrapper] Please install GitHub Copilot CLI or ensure it's in your PATH" >&2
        exit 1
    fi
}
