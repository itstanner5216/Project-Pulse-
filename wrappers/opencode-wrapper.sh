#!/bin/bash
# OpenCode CLI Wrapper with Project-Specific Hook Activation
...
main() {
    ...
    # Pass through to actual OpenCode CLI
    if command -v opencode >/dev/null 2>&1; then
        exec opencode "$@"
    else
        echo "[OpenCode Wrapper] Error: 'opencode' command not found" >&2
        echo "[OpenCode Wrapper] Please install OpenCode CLI or ensure it's in your PATH" >&2
        exit 1
    fi
}
